...
export function registerModelTools(
  ...
  server.tool(
    "qlik_get_fields",
    "Get all fields in the Qlik data model",
    {
    ...
    async ({ include_system = false }) => {
      try {
        ...
        const text = fields.map((f: any) =>
          `- ${f.name} [${f.tags?.join(", ") || "no tags"}] (${f.cardinal} values)`
        ).join("\n");
        return {
  ...
  server.tool(
    "qlik_get_tables",
    "Get all tables in the Qlik data model with row counts",
    {},
    async () => {
      try {
        const tables = await sessionManager.getTables();
        const text = tables.map((t: any) =>
          `- ${t.name}: ${t.rows} rows, ${t.fields} fields, ${t.keyFields} keys`
        ).join("\n");
        return {
  ...
  server.tool(
    "qlik_get_associations",
    "Get associations (links) between tables in the data model",
    {},
    async () => {
      try {
        const assoc = await sessionManager.getAssociations();
        const text = assoc.map((a: any) =>
          `- ${a.table1} <-> ${a.table2} via [${a.keyField}]`
        ).join("\n");
        return {
  ...
  server.tool(
    ...
    async ({ table_name }) => {
      try {
        ...
        const text = schema.fields.map((f: any) =>
          `- ${f.name}: ${f.type || "unknown"} (${f.distinct} distinct)`
        ).join("\n");
        return {
  ...
  server.tool(
    "qlik_get_measures",
    "Get all master measures with their definitions",
    {},
    async () => {
      try {
        const measures = await sessionManager.getMasterMeasures();
        const text = measures.map((m: any) =>
          `- ${m.title}: ${m.expression}\n  ${m.description || ""}`
        ).join("\n");
        return {
  ...
  server.tool(
    "qlik_get_dimensions",
    "Get all master dimensions with their definitions",
    {},
    async () => {
      try {
        const dims = await sessionManager.getMasterDimensions();
        const text = dims.map((d: any) =>
          `- ${d.title}: ${d.field || d.expression}\n  ${d.description || ""}`
        ).join("\n");
        return {
  ...
  server.tool(
    "qlik_get_object_props",
    "Get properties and metadata for a visualization object",
    {
  ...
  server.tool(
    "qlik_get_script",
    "Get the data load script (if accessible)",
    {},
    async () => {
      try {
