...
export function registerConnectionTools(
  ...
  server.tool(
    "qlik_status",
    "Get current connection status to Qlik Sense via Chrome extension",
    {},
    async () => {
      try {
  ...
  server.tool(
    "qlik_list_apps",
    "List all available Qlik Sense apps",
    {},
    async () => {
      try {
  ...
  server.tool(
    "qlik_close_app",
    "Close the current Qlik Sense app",
    {
