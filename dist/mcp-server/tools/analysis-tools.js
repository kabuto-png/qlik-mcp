 ...
 */
import { z } from "zod";
export function registerAnalysisTools(server, sessionManager) {
    // Top N analysis
    server.tool("qlik_top_n", "Get top or bottom N values by an expression", {
        ...
        try {
            ...
            const rows = result.map((r, i) => `${i + 1}. ${r.dimension}: ${r.value}`).join("\n");
            return {
    ...
    server.tool("qlik_crosstab", "Create a cross-tab (pivot) analysis", {
        ...
        try {
            const result = await sessionManager.getCrosstab(row_dimension, col_dimension, expression, max_rows, max_cols);
            // Format as markdown table
            const headers = ["", ...result.columns];
            const headerRow = `| ${headers.join(" | ")} |`;
            const separator = `|${headers.map(() => "---").join("|")}|`;
            const dataRows = result.rows.map((r) => `| ${r.label} | ${r.values.join(" | ")} |`).join("\n");
            return {
    ...
    // Detect duplicates
    server.tool("qlik_detect_duplicates", "Find duplicate values in a field or field combination", {
        ...
        try {
            ...
            const rows = result.slice(0, 50).map((r) => `- ${r.value}: ${r.count} occurrences`).join("\n");
    ...
    // Find outliers
    server.tool("qlik_find_outliers", "Find statistical outliers in a numeric field", {
        ...
        try {
            ...
            const result = await sessionManager.findOutliers(field_name, dimension, method, threshold || defaultThreshold);
            ...
            const outliers = result.outliers.slice(0, 20).map((o) => `- ${dimension ? o.dimension + ": " : ""}${o.value}`).join("\n");
            return {
    ...
    server.tool("qlik_select_by_expression", "Select values in a field that match an expression condition", {
    ...
    // Search and select
    server.tool("qlik_search_select", "Search for values matching a pattern and select them", {
    ...
    // Rank analysis
    server.tool("qlik_rank", "Get ranking of dimension values by expression", {
        ...
        try {
            ...
            const rows = result.map((r) => `#${r.rank} ${r.dimension}: ${r.value}`).join("\n");
            return {
