 ...
 */
import { z } from "zod";
export function registerQcTools(server, sessionManager) {
    // Verify a value exists in a field
    server.tool("qlik_verify_value", "Check if a specific value exists in a Qlik field", {
    ...
    server.tool("qlik_compare", "Compare two Qlik expressions and check if they match", {
        ...
        try {
            ...
            const val1 = parseFloat(result1);
            const val2 = parseFloat(result2);
...
- Match: ${match ? "YES" : "NO"}${tolerance > 0 ? ` (tolerance: ${tolerance})` : ""}`
    ...
    server.tool("qlik_count_nulls", "Count null or empty values in a field", {
        ...
        try {
            ...
            const nullVal = parseInt(nulls) || 0;
...
- Quality: ${qualityPct}%`
    ...
    server.tool("qlik_validate_sum", "Validate that a sum/aggregation matches expected value", {
...
- Tolerance: ${(tolerance_pct * 100).toFixed(1)}%`
    ...
    server.tool("qlik_search", "Search for a value across specified fields", {
    ...
    server.tool("qlik_distinct_count", "Get count of distinct values in a field", {
    ...
    server.tool("qlik_field_stats", "Get statistics for a numeric field (min, max, avg, sum)", {
        ...
        try {
            ...
            const results = await Promise.all(Object.entries(expressions).map(async ([key, expr]) => {
...
//# sourceMappingURL=qc-tools.js.map