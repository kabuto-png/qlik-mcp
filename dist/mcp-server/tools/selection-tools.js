 ...
 */
import { z } from "zod";
export function registerSelectionTools(server, sessionManager) {
    // Get current selections
    server.tool("qlik_get_selections", "Get current selections in the Qlik app", {}, async () => {
        try {
    ...
    server.tool("qlik_select", "Select specific values in a Qlik field", {
    ...
    server.tool("qlik_clear_field", "Clear selections in a specific field", {
    ...
    server.tool("qlik_clear_all", "Clear all selections in the Qlik app", {}, async () => {
        try {
    ...
    // Lock field
    server.tool("qlik_lock_field", "Lock selections in a field to prevent changes", {
    ...
    // Unlock field
    server.tool("qlik_unlock_field", "Unlock a previously locked field", {
    ...
    server.tool("qlik_forward", "Go forward in selection history", {}, async () => {
        try {
    ...
    server.tool("qlik_back", "Go back in selection history", {}, async () => {
        try {
